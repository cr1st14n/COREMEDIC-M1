...
$(function () {
    ...
    function cargarPrestamos() {
        $.get('/COREMEDIC-M1/Recepcion/PresHCL/list/', function (prest) {
            listPrestamos(prest);
        }).fail(function () {
    ...
    function cargarNotas(){
        ...
        $.get('/COREMEDIC-M1/api/listNotasDelDia/'+User+'', function (listNotas) {
            $('#tableNotas').html("");
    ...
    function registrarNota() {
        ...
        if (nota == null || nota.length == 0 || /^\s+$/.test(nota)){
            ...
            var url = "/COREMEDIC-M1/Recepcion/Notas/create";
...
// ---------------funcion para modificar nota
    function notaEdit(x) {
        ...
        $.get('/COREMEDIC-M1/api/buscNota/'+x+'',function (nota){
    ...
    function notaActualizar() {
        ...
        var url = "/COREMEDIC-M1/Recepcion/Notas/update";
        var data= {'_token': $('meta[name=csrf-token]').attr('content'),
            'nota':nota, 'id' :notaID };
    ...
    function notaEliminar(x) {
        $.get('/COREMEDIC-M1/Recepcion/Notas/destroy/'+x+'', function (listNotas) {
            if (listNotas == "error" ){
                var data = new Array();
                data.verticalEdge='right';
                data.horizontalEdge='top';
...
//----------FUNCIONES PARA REGISTRO DE PRESTAMOS
    ...
    function filtrarPrestamos() {
       ...
       var data= {'_token': $('meta[name=csrf-token]').attr('content'),
           ...
           'fecha':fecha ,
           'estado':estado };
       var url = "/COREMEDIC-M1/Recepcion/PresHCL/listFiltrado";
    ...
    function prestEdit(x) {
        $.get('/COREMEDIC-M1/Recepcion/PresHCL/show/'+x+'',function (prest) {
    ...
    function cerrarPrestamo() {
        ...
        $.get('/COREMEDIC-M1/Recepcion/PresHCL/cerrarPrestamo/'+id+'',function (result) {
            console.log(result);
            if (result == 1){
                cargarPrestamos();
                $("#md-editPres").modal('hide');
    ...
    function filtrarNotas() {
        ...
        if (fechaAFiltrar == null || fechaAFiltrar.length == 0 || /^\s+$/.test(fechaAFiltrar)){
            ...
            var url = "/COREMEDIC-M1/Recepcion/Notas/filtrarPrestamos";
            $.post(url,data).done(function (listNotas) {

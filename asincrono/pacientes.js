...
function registrarPrestamo() {
  ...
  if (
    ...
    var url = "/COREMEDIC-M1/Recepcion/PresHCL/create";
    var data = {
...
function rutaAtender(x) {
  //window.alert(x);
  document.location.href = "/COREMEDIC-M1/Recepcion/atencion/index/" + x + "";
}
...
function rutaEditPaciente(x) {
  //indow.alert(x);
  document.location.href = "/COREMEDIC-M1/Recepcion/paciente/edit/" + x + "";
}
function rutaDestroyPaHcl(x) {
  //window.alert(x);
  document.location.href = "/COREMEDIC-M1/Recepcion/paciente/delete/" + x + "";
}
...
function buscarNOMBRESpaciente() {
  ...
  if (nombres == "") {
    ...
    $.get(
      "/COREMEDIC-M1/api/buscarPacienteNombre/" + nombres + "",
      function (paciente) {
        listPacientes(paciente);
        /*$('#resulBusqPacientes').html("");
...
function showEstPres(prest) {
  console.log(prest);
  $.get("/COREMEDIC-M1/Recepcion/PresHCL/show/" + prest + "", function (prest) {
...
function updatePrestamo() {
  ...
  var url = "/COREMEDIC-M1/Recepcion/PresHCL/update";
  var data = {
    _token: $("meta[name=csrf-token]").attr("content"),
    id: id,
    area: area,
...
function cerrarPrestamo() {
  ...
  $.get(
    "/COREMEDIC-M1/Recepcion/PresHCL/cerrarPrestamo/" + id + "",
    function (result) {
      console.log(result);
      if (result == 1) {
        $("#md-editPres").modal("hide");
...
function showAtender(id) {
  ...
  $.ajax({
    ...
    success: function (r) {
      ...
      htmlDatCitPrevPaci = r.ate.map(function (d) {
          return (a = `
            <tr>
              <td style="font-size:9">${moment(d.created_at).format("DD/MM/YYYY HH:mm")}</td>
              <td>${d.usu_nombre} ${d.usu_appaterno}</td>
              <td>${d.esp_nombre} -> ${d.ate_descripcion}</td>
              <td></td>
            </tr>
          `);
        })
      ...
      $("#tableBoContCitasAnt").html('');
      $("#tableBoContCitasAnt").html(htmlDatCitPrevPaci);

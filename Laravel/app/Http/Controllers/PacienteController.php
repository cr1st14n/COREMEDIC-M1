...
class PacienteController extends Controller
{
  ...
  public function create1(Request $request)
  {
    ...
    $resul = $paciente->save();
    if ($resul) {
      return ['res' => 1, 'data' => $paciente];
    } else {
      return ['res' => 0, 'data' => $paciente];
    }
  }
  ...
  public function storePa1(Request $request)
  {
    ...
    //? ----- especialidades
    $esp = especialidad::select('id', 'esp_nombre', 'esp_detalle', 'esp_costo')->get();
    ...
    //? --  Procedimieto previo citas medicas
    $atencion = atencion::where('pa_id', $request->input('id'))
      ->join('especialidad as espe', 'atencion.ate_especialidad', '=','espe.id')
      ->join('users', 'atencion.ate_med', '=','users.id')
      ->select('esp_nombre','atencion.created_at','usu_nombre','usu_appaterno','ate_descripcion' )
      ->get();

    return ['pa' => $paciente, 'edad' => $edad, 'sex' => $sex, 'esp' => $esp, 'med' => $med, 'ate' => $atencion];
  }
}

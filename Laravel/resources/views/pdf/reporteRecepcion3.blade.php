		...
		<div class="row">
			<section class="panel corner-flip">
				<div class="panel-body">
					...
					<table cellpadding="0" cellspacing="0" border="0" width=100% class="" id="" style="text-transform:capitalize; ;">
						<thead>
							<tr>
						...
						<tbody align="center" style="text-transform: lowercase;">
							...
							<tr>
								<td>
									<h6>{{ $atencion->pa_hcl }}</h6>
								</td>
								<td>
									<h6>{{ $atencion->pa_ci}} </h6>
								</td>
								<td>
									<h6><strike>{{ $atencion->pa_nombre}} {{ $atencion->pa_appaterno }} {{ $atencion->pa_apmaterno }}</strike> </h6>
								</td>
								<td>
									<h6><strike>{{ $atencion->ps_appaterno }} {{ $atencion->ps_apmaterno }} {{ $atencion->ps_nombre }}</strike></h6>
								</td>
								<td>
									<h6><strike>{{ $atencion->nombre }}</strike></h6>
								</td>
								<td>
									<h6><strike>{{ $atencion->ate_pago}}</strike></h6>
								</td>
							</tr>
							@else
							<tr>
								...
								<td>
									<h6>{{ $atencion->pa_nombre}} {{ $atencion->pa_appaterno }} {{ $atencion->pa_apmaterno }} </h6>
								</td>
								<td>
									<h6>{{ $atencion->usu_appaterno }} {{ $atencion->usu_apmaterno }} {{ $atencion->usu_nombre }}</h6>
								</td>
								<td>
									<h6>{{ $atencion->esp_nombre }} - {{ $atencion->esp_detalle }} </h6>
								</td>
								<td align="right">
									<h6>{{ $atencion->esp_costo}}</h6>
								</td>
							</tr>
							@endif
							@endforeach
					...
					<table cellpadding="0" cellspacing="0" border="0" class="" id="" style="text-transform:capitalize; ;">
						...
						<tbody align="center" style="text-transform: lowercase;">
							...
							<tr>
								{{--<td><h6>-{{$nota->created_at}}</h6>
								</td>--}}
								<td width="10%">
									<h6><?php print(date("H:i", strtotime($nota->created_at))) ?> </h6>
								</td>
								<td>
									<h6>-{{$nota->rn_nota}}</h6>
								</td>
